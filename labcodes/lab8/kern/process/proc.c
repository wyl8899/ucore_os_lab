...
alloc_proc(void) {
     ...
     */
     //LAB5 2014011434 : (update LAB4 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2014011434 HINT:need add some code to init fs in proc_struct, ...
        ...
        memset(&(proc->lab6_run_pool), sizeof(skew_heap_entry_t), 0);
        proc->time_slice = 0;
        proc->lab6_stride = 0;
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //LAB8:EXERCISE2 2014011434  HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2014011434  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    int phnum_i = 0;
    for (phnum_i = 0; phnum_i < elf->e_phnum; ++phnum_i) {
        if ((ret = load_icode_read(fd, ph, sizeof(struct proghdr),
                        elf->e_phoff + phnum_i * sizeof(struct proghdr))) != 0) {
    ...
    //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
        ...
        uint32_t from_offset = ph->p_offset;
        size_t off, size;
     ...
     //(3.6.1) copy TEXT/DATA section of bianry program
        while (start < end) {
            ...
            if ((ret = load_icode_read(fd, page2kva(page) + off,
                            size, from_offset)) != 0) {
                goto bad_cleanup_mmap;
            }
    ...
    uintptr_t ustacktop = (uintptr_t)USTACKTOP;
    {
        uint32_t arg_size = 0;
        int i;
        for (i = 0; i < argc; ++i) {
            arg_size += strlen(kargv[i]) + 1;
        }
        int align_to = sizeof(char*);
        ustacktop -= (arg_size + align_to - 1) / align_to * align_to;
        char* args = ustacktop;
        ustacktop -= argc * sizeof(char*);
        char** argv = (char **)ustacktop;
        ustacktop -= sizeof(int);
        *(int *)ustacktop = argc;
        arg_size = 0;
        for (i = 0; i < argc; ++i) {
            argv[i] = args + arg_size;
            strcpy(argv[i], kargv[i]);
            arg_size += strlen(kargv[i]);
        }
    }
